...
export const BlogPostDetail = () => {
  ...
  const fetchPost = async () => {
    try {
      ...
      if (response.success) {
        // BlogDataType now covers all BlogPost fields; cast via unknown is safe
      ...
      setError(err.message ?? null);           // fix: ?? null
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchComments = async () => {
    try {
      ...
      if (response.success) {
        setComments((response.data ?? []) as unknown as Comment[]);  // fix: unknown cast
      }
    } catch (err) {
