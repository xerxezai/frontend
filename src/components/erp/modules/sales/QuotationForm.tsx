...
export interface QuotationFormValues {
  number: string; customer: string; issue_date: string;
  status: string; valid_until: string; notes: string;
  items: QuotationItemRow[];
}

const emptyItem: QuotationItemRow = { product: '', description: '', quantity: '1', unit_price: '' };

export default function QuotationForm({ initial, editing, onClose, onSave }: {
  initial: QuotationFormValues;
  editing: boolean;
  onClose: () => void;
  onSave: (values: QuotationFormValues) => Promise<void>;
}) {
  ...
  const set = (k: keyof QuotationFormValues, v: any) => setF(p => ({ ...p, [k]: v }));

  const setItem = (i: number, patch: Partial<QuotationItemRow>) =>
  ...
  const submit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!f.number.trim())  { toast.error('Quotation number is required.'); return; }
        ...
        <form onSubmit={submit} style={{ display:'flex',flexDirection:'column',gap:14 }}>
          <div className="row g-3" style={{ margin: 0 }}>
            ...
            <div className="col-md-6" style={{ padding: '12px 0 0' }}>
              ...
              <input type="date" value={f.valid_until} onChange={e => set('valid_until', e.target.value)} style={inp} />
            </div>
            <div className="col-md-6" style={{ padding: '12px 8px 0 0' }}>
          ...
          <div>
            ...
            <div style={{ border:'1px solid rgba(0,0,0,0.08)', borderRadius: 10, overflow: 'hidden' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12.5, fontFamily: 'DM Sans,sans-serif' }}>
                ...
                <tbody>
                  {f.items.length === 0 && (
                    <tr><td colSpan={5} style={{ padding: '16px 10px', textAlign: 'center', color: '#9b9690' }}>No items — add at least one, or leave empty for a blank quotation.</td></tr>
                  )}
                  {f.items.map((it, i) => {
