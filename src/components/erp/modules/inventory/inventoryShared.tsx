...
export const MOVEMENT_TYPES: Record<string, { label: string; bg: string; color: string }> = {
  in:       { label: 'IN',         bg: '#d1fae5', color: '#065f46' },
  out:      { label: 'OUT',        bg: '#fee2e2', color: '#991b1b' },
  transfer: { label: 'TRANSFER',   bg: '#ede9fe', color: '#6d28d9' },
  adjust:   { label: 'ADJUSTMENT', bg: '#dbeafe', color: '#1d4ed8' },
  return:   { label: 'RETURN',     bg: '#fef3c7', color: '#92400e' },
  damage:   { label: 'DAMAGE',     bg: '#ffe4e6', color: '#9f1239' },
};

export const MovementBadge = ({ type }: { type: string }) => {
  const m = MOVEMENT_TYPES[type] ?? { label: type, bg: '#f1f5f9', color: '#64748b' };
  return (
    <span style={{ display:'inline-block',padding:'2px 10px',borderRadius:20,fontSize:10.5,fontWeight:700,background:m.bg,color:m.color,fontFamily:FF,whiteSpace:'nowrap' }}>
      {m.label}
    </span>
  );
};
