...
export const PO_STATUS: Record<string, { label: string; bg: string; color: string }> = {
...
export const BILL_STATUS: Record<string, { label: string; bg: string; color: string }> = {
...
/** Bill status badge — "Overdue" takes precedence over the stored status when is_overdue is
 *  true, since the backend never mutates status to 'overdue' itself (no background scheduler). */
export const BillStatusBadge = ({ status, isOverdue }: { status: string; isOverdue?: boolean }) => (
  <StatusBadge status={isOverdue ? 'overdue' : status} map={BILL_STATUS} />
);

export const StarRating = ({ value }: { value: number }) => (
  <span style={{ display: 'inline-flex', gap: 1, color: OG, fontSize: 12 }} aria-label={`${value} out of 5 stars`}>
    {[1, 2, 3, 4, 5].map(n => (
      <i key={n} className={n <= value ? 'fas fa-star' : 'far fa-star'} style={{ color: n <= value ? OG : '#d1d5db' }} />
    ))}
  </span>
);

export const StarRatingInput = ({ value, onChange }: { value: number; onChange: (n: number) => void }) => (
  <span style={{ display: 'inline-flex', gap: 4 }}>
    {[1, 2, 3, 4, 5].map(n => (
      <button
        ...
        style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 4, fontSize: 18, color: n <= value ? OG : '#d1d5db' }}
      >
