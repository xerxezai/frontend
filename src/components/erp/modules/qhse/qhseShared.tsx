...
export const SEVERITY_BADGE: Record<string, { label: string; bg: string; color: string }> = {
  low:      { label: 'Low',      bg: '#d1fae5', color: '#065f46' },
  medium:   { label: 'Medium',   bg: '#fef3c7', color: '#92400e' },
  high:     { label: 'High',     bg: '#fee2e2', color: '#991b1b' },
  critical: { label: 'Critical', bg: '#7f1d1d', color: '#fff' },
};

export const INCIDENT_STATUS: Record<string, { label: string; bg: string; color: string }> = {
  open:             { label: 'Open',             bg: '#fee2e2', color: '#991b1b' },
  investigating:    { label: 'Investigating',    bg: '#fef3c7', color: '#92400e' },
  action_required:  { label: 'Action Required',  bg: '#ffedd5', color: '#c2410c' },
  closed:           { label: 'Closed',           bg: '#e5e7eb', color: '#374151' },
  resolved:         { label: 'Resolved',         bg: '#d1fae5', color: '#065f46' },
};

export const INSPECTION_STATUS: Record<string, { label: string; bg: string; color: string }> = {
...
export const RISK_LEVEL_BADGE = SEVERITY_BADGE;

export const RISK_STATUS: Record<string, { label: string; bg: string; color: string }> = {
  open:      { label: 'Open',      bg: '#fee2e2', color: '#991b1b' },
  mitigated: { label: 'Mitigated', bg: '#fef3c7', color: '#92400e' },
  closed:    { label: 'Closed',    bg: '#e5e7eb', color: '#374151' },
  accepted:  { label: 'Accepted',  bg: '#d1fae5', color: '#065f46' },
};

export const CHECKLIST_STATUS: Record<string, { label: string; bg: string; color: string }> = {
  pending:   { label: 'Pending',   bg: '#fef3c7', color: '#92400e' },
  completed: { label: 'Completed', bg: '#d1fae5', color: '#065f46' },
  failed:    { label: 'Failed',    bg: '#fee2e2', color: '#991b1b' },
};

export const COMPLIANCE_STATUS: Record<string, { label: string; bg: string; color: string }> = {
  compliant:            { label: 'Compliant',            bg: '#d1fae5', color: '#065f46' },
  non_compliant:        { label: 'Non-Compliant',        bg: '#fee2e2', color: '#991b1b' },
  partially_compliant:  { label: 'Partially Compliant',  bg: '#fef3c7', color: '#92400e' },
