...
export const ProgressBar = ({ value, height = 8, accent = OG }: { value: number; height?: number; accent?: string }) => {
  const pct = Math.max(0, Math.min(100, value || 0));
  const isDefaultAccent = accent === OG;
  const fill = isDefaultAccent ? (pct >= 100 ? '#10b981' : OG_G) : accent;
  return (
    <div style={{ width: '100%', height, borderRadius: height, background: '#F0EBE4', overflow: 'hidden' }}>
      <div style={{ width: `${pct}%`, height: '100%', borderRadius: height, background: fill, transition: 'width 320ms cubic-bezier(0.22,1,0.36,1)' }} />
    </div>
  );
};

export const PROJECT_STATUS: Record<string, { label: string; bg: string; color: string }> = {
...
export const MILESTONE_STATUS: Record<string, { label: string; bg: string; color: string }> = {
...
export const TASK_STATUS: Record<string, { label: string; bg: string; color: string }> = {
...
export const BUDGET_CATEGORY_LABEL: Record<string, string> = {
  labour: 'Labour', materials: 'Materials', equipment: 'Equipment',
  subcontractor: 'Subcontractor', overhead: 'Overhead', other: 'Other',
};

export const initials = (name?: string | null) => (name || '').trim().split(/\s+/).map(w => w[0]).slice(0, 2).join('').toUpperCase() || '?';
