      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: FF, fontSize: 12.5 }}>
        ...
        <tbody>
          {entries.map(e => {
            ...
            return (
              <tr key={e.id} style={{ borderBottom: '1px solid rgba(0,0,0,0.05)', background: isCurrent ? 'rgba(217,53,34,0.06)' : undefined }}>
                ...
                <td style={{ padding: '9px 10px', fontWeight: isCurrent ? 800 : 700, color: isCurrent ? '#D93522' : '#1A1A1A' }}>{fmtINR(e.closing_value)}</td>
              </tr>
            );
          })}
        </tbody>
