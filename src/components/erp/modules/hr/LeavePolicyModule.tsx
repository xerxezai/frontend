...
function EditPolicyModal({ policy, onClose, onSaved }: { policy: any; onClose: () => void; onSaved: (body: any) => Promise<void> }) {
  ...
  return (
    <div style={{ position:'fixed',inset:0,zIndex:1050,background:'rgba(0,0,0,0.40)',backdropFilter:'blur(3px)',display:'flex',alignItems:'center',justifyContent:'center',padding:16 }} onClick={onClose}>
      <div onClick={e=>e.stopPropagation()} style={{ background:'#fff',borderRadius:14,padding:'28px 24px 24px',maxWidth:420,width:'100%',boxShadow:'0 20px 60px rgba(0,0,0,0.16)',borderTop:'3px solid #D93522' }}>
                ...
                {sorted.map((p: any) => {
                  ...
                  return (
                    <tr key={p.id} style={{ borderBottom: `1px solid ${BORDER}`, opacity: p.is_active ? 1 : 0.55 }}>
                      ...
                      {isAdmin && (
                        <td style={{ padding: '11px 16px' }}>
                          <button onClick={()=>setEditing(p)} style={{ background:'rgba(217,53,34,0.08)', color:OG, border:'1px solid rgba(217,53,34,0.22)', width:28, height:28, display:'flex', alignItems:'center', justifyContent:'center', borderRadius:6, cursor:'pointer' }}>
                            <Pencil size={12} />
                          </button>
                        </td>
                      )}
